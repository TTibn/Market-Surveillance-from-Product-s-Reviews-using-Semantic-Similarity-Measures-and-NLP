...
# Defining the function to send a review to ChatGPT
def send_review_to_chatgpt(review, api_key):
...
# Sending reviews to ChatGPT one by one
api_key = ''
y_pred = []

for review in reviews_list:
    sentiment = send_review_to_chatgpt(review, api_key)
...
if len(y_pred) == len(y_true):
    ...
    plt.title('ROC Curve GPT-3.5 turbo')
    plt.show()
    ...
    plt.title('Precision-Recall Curve GPT-3.5 turbo')
    plt.show()
