...
def calculate_weighted_average_semantic_similarity(tweet, danger_keywords, method, brown_ic):
    ...
    # Calculation of the total weighted similarity score
    if method == "wup":
        for normalized_score in normalized_scores:
            weight = 0.0
            if normalized_score > 0.5:
                weight = 0.102854668591712
            elif normalized_score > 0.4 and normalized_score <= 0.5:
                weight =0.2665385989323752
            # elif normalized_score >= 0.4 and normalized_score < 0.6:
            #     weight = 0.234016
            elif normalized_score > 0.308 and normalized_score <= 0.4:
                weight =0.3233983496920063
            # elif normalized_score >= 0.2 and normalized_score < 0.4:
            #     weight = 0.9996004225105434
            else:
                weight = 0.3372083827839063
            total_sem_sim += normalized_score * weight
            total_weight += weight

    elif method == "res":
        for normalized_score in normalized_scores:
            weight = 0.0
            if normalized_score >= 0.0001:
                weight = 0.3672329067384936
            else:
                weight = 0.7327670932615064
            total_sem_sim += normalized_score * weight
            total_weight += weight
        ...
        for normalized_score in normalized_scores:
            weight = 0.0
            if normalized_score >= 0.0679:
                weight = 0.2069335702324975
            elif normalized_score >= 0.0465 and normalized_score <= 0.0678:
                weight =0.5346678511624875
            else:
                weight = 0.358398578605015
            total_sem_sim += normalized_score * weight
            total_weight += weight
        ...
        for normalized_score in normalized_scores:
            weight = 0.0
            if normalized_score >= 0.3311:
                weight = 0.1476203982449991
            elif normalized_score >= 0.2241 and normalized_score <= 0.331:
                weight =0.2381019912249956
            elif normalized_score >= 0.0759 and normalized_score <= 0.224:
                weight =0.2381019912249956
            elif normalized_score >= 0.0529 and normalized_score <= 0.0758:
                weight = 0.2381303645116252
            else:
                weight = 0.2380452547933841
            total_sem_sim += normalized_score * weight
            total_weight += weight
        ...
        for normalized_score in normalized_scores:
            weight = 0.0
            if normalized_score >= 0.3331:
                weight = 0.1836239363006592
            elif normalized_score >= 0.2001 and normalized_score <= 0.333:
                weight =0.2315731754085826
            elif normalized_score >= 0.1251 and normalized_score <= 0.2:
                weight =0.3503191632661774
            else:
                weight = 0.3344837250245807
            total_sem_sim += normalized_score * weight
            total_weight += weight
        ...
        for normalized_score in normalized_scores:
            weight = 0.0
            if normalized_score > 0.5711:
                weight = 0.2923186551296787
            elif normalized_score >= 0.4641 and normalized_score <= 0.571:
                weight =0.1907792744739755
            else:
                weight = 0.6169020703963458
            total_sem_sim += normalized_score * weight
            total_weight += weight
...
for method in methods:
    ...
    plt.legend(loc='best')
    plt.grid(True)
    plt.savefig(f"ROC_Curve_for_{method}.jpg")
    plt.show()
